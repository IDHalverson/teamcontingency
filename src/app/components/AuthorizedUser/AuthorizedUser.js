...
const AuthorizedUser = ({ isPrimary, size: { width } }) => {
  const AuthorizedUserPrimaryView = (
    <div>
      <FullPaneLeftHeader
        title={"User 1"}
  ...
  const AuthorizedUserSecondaryView = (
    <div>
      <FullPaneLeftHeader
        title={"User 1"}
        text={"Thanks for choosing Capital One!"}
  ...
  if (isPrimary) return AuthorizedUserPrimaryView;
  if (!isPrimary) return AuthorizedUserSecondaryView;
};
